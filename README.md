# 餐盘有数 · LXJ Meal Planner

[![CI](https://github.com/Maj1sty/lxj-meal-planner/actions/workflows/ci.yml/badge.svg)](https://github.com/Maj1sty/lxj-meal-planner/actions/workflows/ci.yml)
[![Deploy Pages](https://github.com/Maj1sty/lxj-meal-planner/actions/workflows/pages.yml/badge.svg)](https://github.com/Maj1sty/lxj-meal-planner/actions/workflows/pages.yml)
[![License: MIT](https://img.shields.io/badge/Code%20License-MIT-173f34.svg)](LICENSE)
[![Data: attributed](https://img.shields.io/badge/Data-attributed-e96842.svg)](DATA_LICENSE.md)

一个基于公开营养信息、支持离线使用的餐品营养计算工具。它可以自由组合餐品，计算一餐的热量、蛋白质、脂肪、碳水和已知钠值。

项目最重要的原则是：**缺失值不按零计算，每个数字都能回到来源。**

> 非官方项目，与老乡鸡及 FatSecret 无隶属、授权或合作关系。结果仅作一般信息与软件演示，不替代包装标签、医生或注册营养师建议。

## 在线体验

发布后访问：<https://maj1sty.github.io/lxj-meal-planner/>

## 功能

- 按全部、主食、主菜、蔬菜和汤品筛选或搜索菜品。
- 以 0.5 倍增量自由组合餐品。
- 计算热量、蛋白质、脂肪、碳水、钠和食盐当量。
- 任一已选餐品缺少钠时，整餐钠值保持未知，不会错误显示为零。
- 显示数据来源、访问日期、口径、完整度与已知限制。
- 支持复制本餐摘要。
- 首次在线打开后，Service Worker 会缓存应用外壳，后续可离线打开和计算。
- 数据说明页面展示每个公开版本的更新内容。
- 支持设置 800～6000 kcal 的每日热量目标，显示本餐占比与剩余额度。
- 使用合肥信达好第坊店作为菜单展示模板，并明确标注实时在售状态待核验。

## 当前数据状态

当前收录 98 条可追溯餐品数据，其中 87 条由 CookLikeHOC 根据《老乡鸡菜品溯源报告 2.0》整理。

| 项目 | 覆盖 | 说明 |
|---|---:|---|
| 热量、蛋白质、脂肪、碳水 | 98/98 | 第三方公开页面与溯源报告开源整理 |
| 钠 | 95/98 | 其余 3 条保持缺失 |
| 食盐当量 | 95/98 | 仅在所选餐品钠值完整时计算 |
| 官方营养值逐条交叉核验 | 0/98 | 仍需从官方营养标识逐条核对 |
| 每份克重 | 1/2 份制菜品 | 三黑元气饭只有“每份”口径 |
| 实时菜单与地区供应 | 未覆盖 | 本项目不是实时菜单 |

更完整的说明见 [数据缺口](docs/data-gaps.md) 和 [数据方法](docs/data-methodology.md)。

## 快速开始

需要 Node.js 20 或更高版本。

```bash
git clone https://github.com/Maj1sty/lxj-meal-planner.git
cd lxj-meal-planner
npm install
npm run dev
```

浏览器打开 <http://127.0.0.1:4173>。

项目没有运行时第三方依赖，`npm install` 主要用于生成一致的锁文件环境。

## 验证

```bash
npm run check
npm test
npm run build
```

测试覆盖：

- 数量与营养合计；
- 缺失钠值传播；
- 食盐当量不被错误生成；
- 数据 ID、来源和字段质量。
- Service Worker 应用外壳与离线回退规则。

## 离线使用

Service Worker 仅在 HTTPS 或本机开发地址上生效。首次在线访问会缓存页面、样式、脚本、餐品数据、图标和应用清单；之后即使断网，已访问过的版本仍可打开并完成营养计算。外部来源链接不做缓存。

## 数据来源

1. [老乡鸡官方微博：菜品溯源报告 2.0 线上可视化说明](https://www.sina.cn/news/detail/5296041042647572.html)
2. [FatSecret：老乡鸡产品营养信息](https://www.fatsecret.cn/热量营养/老乡鸡)
3. [新京报转载稿：溯源报告 2.0 菜品营养成分整理](https://www.bjnews.com.cn/detail/1782531912129036.html)
4. [CookLikeHOC：金牌老母鸡汤开源菜谱](https://bazaar.fcc-cd.dev/recipe/%E6%B1%A4/%E9%87%91%E7%89%8C%E8%80%81%E6%AF%8D%E9%B8%A1%E6%B1%A4.md)
5. [CookLikeHOC：溯源报告餐品开源整理仓库](https://github.com/Gar-b-age/CookLikeHOC)
6. [老乡鸡官网：菜品溯源报告 2.0](https://lxjchina.com.cn/info.php?class_id=102106)
7. [国家卫健委：餐饮食品营养标识指南](https://www.nhc.gov.cn/sps/c100088/202012/9f71d532e5684a54a63090a75eb737fb/files/1732844456897_92407.pdf)

食盐当量在钠数据可用时采用指南口径：

```text
食盐当量（g） = 钠（mg） × 2.5 ÷ 1000
```

FatSecret 数据仅用于小规模、带来源链接的演示记录。若后续批量接入，应使用其正式 API 并遵守署名与使用条款。

## 项目结构

```text
.
├── src/
│   ├── core/              # 营养计算与缺失值规则
│   ├── data/              # 菜品数据与来源元信息
│   ├── app.js             # 页面状态与交互
│   └── styles.css         # 响应式界面
├── tests/                 # Node 原生测试
├── scripts/               # 本地服务、构建与检查
├── data/                  # 可审阅的 CSV 数据快照
├── docs/                  # 架构、数据方法和缺口
├── service-worker.js      # 应用外壳缓存与离线回退
├── manifest.webmanifest   # 可安装 Web 应用信息
├── .github/               # CI、Pages、Issue 与 PR 模板
└── index.html
```

架构说明见 [docs/architecture.md](docs/architecture.md)。

## Roadmap

- [x] 自由选餐和营养合计
- [x] 数据来源与缺失值展示
- [x] 扩充至 20 条可追溯餐品
- [x] Service Worker 离线缓存
- [x] 扩充至 98 条可追溯餐品
- [x] 自定义每日热量目标
- [x] 自动化测试和 GitHub Actions
- [x] GitHub Pages 部署
- [ ] 逐条核验官方营养标识
- [ ] 补全钠和真实出餐份量
- [ ] 使用门店实拍或点餐页面逐条核验信达好第坊店实时菜单
- [ ] 添加地区、在售状态与数据版本
- [ ] 支持用户导入自定义数据
- [ ] 在合法授权前提下接入正式数据 API

## 参与贡献

欢迎提交数据勘误、来源补充和规则改进。涉及营养值的改动必须附原始来源、份量口径和访问日期，详见 [CONTRIBUTING.md](CONTRIBUTING.md)。

## 许可证

- 源代码使用 [MIT License](LICENSE)。
- 第三方营养事实、名称与商标不因代码许可证而重新授权，见 [DATA_LICENSE.md](DATA_LICENSE.md)。
