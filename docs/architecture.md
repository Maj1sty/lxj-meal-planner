...
当前有 98 条静态数据，不涉及账号、私密信息或服务端任务。数据规模、用户导入或定期同步出现后，再考虑后端。

### 生成数据与人工数据合并

`report-foods.generated.js` 由 CookLikeHOC 的 87 条完整营养记录机械生成，再按菜名与人工核验记录合并。相同菜名优先采用包含五项营养值的报告整理记录，最终得到 98 条去重餐品。

### 每日热量目标只在本机保存

用户设置的热量目标保存在浏览器 `localStorage`，不会上传。当前功能只比较“本餐”与每日目标，不追踪早餐、午餐、晚餐等多餐累计。
